...
#define MAX_CLOUD_WRITES 3      // Max number of writes to the cloud each wake up session
...
//  if (resyncFlag & writeCounter < MAX_CLOUD_WRITES) {  // resynchronization period is over
 if (resyncFlag) {  // resynchronization period is over
   
    if (millis() - writeTimer >= 30000) {  // 30 seconds between writing to the cloud
      //ready to write the message to the cloud
  ...
  if (millis() - wakeTimer > 180000) {
    SerialUSB.println("PROGRAM: End of Wake Period! Time to go to sleep for 10 mins");
    wakeFlag = false;
    resyncFlag = false;

    Dash.deepSleepMin(240);   // deepSleep for 10 mins
//     Dash.snooze(900000);  // snooze for 5 mins
